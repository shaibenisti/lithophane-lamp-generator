...
def load_application_settings(config_dir: Optional[Path] = None) -> Settings:
    ...
    try:
        ...
        logger.debug(f"Bottom margin: {settings.bottom_margin}mm")
        logger.debug(f"Top margin: {settings.top_margin}mm")
        logger.debug(f"Max thickness: {settings.max_thickness}mm")
        return settings
        
    except Exception as e:
        logger.warning(f"Could not load settings from ConfigManager: {e}")
        # Last resort: use defaults
        logger.warning("Using hardcoded default settings")
        return Settings()
