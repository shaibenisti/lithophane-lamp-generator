...
class ImageRescueSystem:
    ...
    def _rescue_low_resolution(self, image: np.ndarray,
        ...
        """
        Rescue low-resolution images using intelligent upscaling.
        ...
        if quality_tier == ImageQualityTier.VERY_POOR:
            ...
            target_min = 1200
            upscale_factor = max(4, target_min / min(width, height))
        elif quality_tier == ImageQualityTier.POOR:
            # Need significant upscaling (2-3x)
            target_min = 1000
            upscale_factor = max(2, target_min / min(width, height))
        else:
            # Minor upscaling (1.5-2x)
            target_min = 1200
            upscale_factor = min(2, target_min / min(width, height))

        # Cap upscale factor to prevent ridiculous sizes
        upscale_factor = min(upscale_factor, 6)

        self.logger.info(f"Upscaling image by {upscale_factor:.2f}x")

        # Use high-quality upscaling method
        # Method selection based on upscale factor
        if upscale_factor >= 3:
            # For heavy upscaling, use edge-preserving method
            upscaled = self._advanced_upscale(image, upscale_factor)
            method = "AI-enhanced upscaling"
        elif upscale_factor >= 2:
            # For moderate upscaling, use Lanczos
            ...
            # For minor upscaling, use bicubic
            new_width = int(width * upscale_factor)
            new_height = int(height * upscale_factor)
            upscaled = cv2.resize(image, (new_width, new_height),
